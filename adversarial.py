...
# def normalize(emb, vocab_freqs):
#     weights = vocab_freqs/tf.reduce_sum(vocab_freqs)
#     mean = tf.reduce_sum(weights * emb,0,keep_dims=True)
#     var = tf.reduce_sum(weights * tf.pow((emb - mean),2),0,keep_dims=True)
#     return((emb - mean)/tf.sqrt(var))
# def scale_l2(x, norm_length):#这个函数的作用就是论文中的x/|x的平方|
...
#         grad = tf.stop_gradient(grad)  # 不会反向传播？
#         perturb = scale_l2(grad, self.epsilon)  # 论文中的eg/|g的平方|
#         return embedded + perturb  # 原本词向量加扰动
...
#             correct_predictions = tf.equal(self.prediction, self.real)
...
def scale_l2(x, norm_length):#这个函数的作用就是论文中的x/|x的平方| 求单位向量？
    # shape(x) = (batch, num_timesteps, d)
    # Divide x by max(abs(x)) for a numerically stable L2 norm.
    # 2norm(x) = a * 2norm(x/a)
    # Scale over the full sequence, dims (1, 2)
...
def normalize(emb, weights): #嵌入层进行归一化
...
class AdversarialClassifier(object):
    def __init__(self, config):
        ...
        # placeholder
        self.x = tf.placeholder(tf.int32, [None, 32])
    ...
    def build_graph(self, vocab_freq, word2idx):
        ...
        def cal_loss_logit(embedded, keep_prob, reuse=True, scope="loss"):
            ...
            return y_hat, tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(logits=y_hat, labels=self.label))

        logits, self.cls_loss = cal_loss_logit(batch_embedded, self.keep_prob, reuse=False)
        embedding_perturbated = self._add_perturbation(batch_embedded, self.cls_loss)
        adv_logits, self.adv_loss = cal_loss_logit(embedding_perturbated, self.keep_prob, reuse=True)
        self.losses = self.cls_loss + self.adv_loss
    ...
    with sess.as_default():
        ...
        for e in range(20):
            ...
            for x_batch, y_batch in fill_feed_dict(x_train, y_train, config['batch_size']):
                ...
                l, _, __ = sess.run([classifier.losses, classifier.train_op, classifier.global_step], feed_dict=fd)
